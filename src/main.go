...
func main() {
    ...
    hostsLinks := make([]string, 0)
    {
        hostThread := func(i int) {
            for j := i; j < len(allBooks); j += cores {
                pathId := allBooks[j].id
                url := strings.Join([]string{"http://23.95.221.108/download.php?id=", pathId}, "")
                htmlBody := get(url)
                hosts <- htmlBody
            }
        }
